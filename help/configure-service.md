...
description: Prepare sua instância de AEM para usar o serviço de conversão automatizada da Forms
...
O serviço de conversão automatizado do Forms é executado AEM instância do autor. É necessário AEM 6.4 ou AEM 6.5 para configurar uma instância do autor AEM. Se você não tiver AEM funcionando, baixe-o dos seguintes locais:

* Se você já for um cliente AEM, baixe AEM 6.4 ou AEM 6.5 do site [de licenciamento da](http://licensing.adobe.com)Adobe.

* Se você for um parceiro de Adobe, use o Programa [de treinamento de parceiro](https://adobe.allegiancetech.com/cgi-bin/qwebcorporate.dll?idx=82357Q) Adobe para solicitar AEM 6.4 ou AEM 6.5.

Depois de baixar o AEM, para obter instruções sobre como configurar uma instância AEM do autor, consulte [implantação e manutenção](https://helpx.adobe.com/experience-manager/6-5/sites/deploying/using/deploy.html#defaultlocalinstall).

### Baixe e instale AEM Service Pack mais recente {#servicepack}

Baixe e instale o AEM Service Pack mais recente. Para obter instruções detalhadas, consulte ou [AEM 6.4 Notas](https://helpx.adobe.com/experience-manager/6-4/release-notes/sp-release-notes.html) de versão do Service Pack ou [AEM 6.5 Notas](https://helpx.adobe.com/experience-manager/6-5/release-notes/sp-release-notes.html)de versão do Service Pack.

### Baixar e instalar o pacote suplementar do AEM Forms  {#downloadaemformsaddon}
...
* **Administradores**: Os administradores são responsáveis pelo gerenciamento de software e serviços de Adobe para sua organização. Os administradores concedem acesso aos desenvolvedores em sua organização para se conectarem ao serviço de conversão automatizada da Forms em execução na Adobe Cloud. Quando um administrador é provisionado para uma organização, o administrador recebe um email com título **[!UICONTROL 'You now have administrator rights to manage Adobe software and services for your organization']**. Se você for um administrador, verifique seu email de caixa de correio com o título acima mencionado e prossiga para [conceder acesso aos desenvolvedores de sua organização](#adduseranddevs).

![email de concessão de acesso do administrador](assets/admin-console-adobe-io-access-grantedx75.png)

* **Desenvolvedores**: Um desenvolvedor conecta uma instância local do autor do AEM Forms ao serviço de conversão automatizada do Forms em execução na Adobe Cloud. Quando um administrador concede direitos a um desenvolvedor para se conectar ao serviço de conversão automatizada da Forms, um email com o título Você agora tem acesso de desenvolvedor para gerenciar integrações da API do Adobe para sua organização é enviado ao desenvolvedor. Se você for um desenvolvedor, verifique se há e-mail na caixa de correio com o título acima mencionado e prossiga para [Conectar sua instância AEM local ao serviço de Conversão automatizada da Forms na Adobe Cloud.](#connectafcadobeio)
...
Os desenvolvedores são membros de sua organização designados para executar o serviço de conversão. Somente os desenvolvedores adicionados ao perfil do serviço de conversão automatizada Adobe Forms têm direito a usar o serviço de conversão automatizada Forms. Execute as etapas abaixo para criar um perfil e adicionar desenvolvedores a ele. É necessário, no mínimo, um perfil para conceder o acesso necessário aos desenvolvedores de sua organização:

1. Faça logon no [Admin Console](https://adminconsole.adobe.com/). Use o **Adobe ID** do administrador provisionado para usar o serviço de Conversão automatizada do Forms para fazer logon. Não faça logon com outra ID ou Federated ID.
...
Quando um administrador adiciona desenvolvedores ao perfil de E/S do Adobe, eles são notificados por email. Depois de receber o email, os desenvolvedores podem continuar a [conectar uma instância local do AEM Forms com o serviço de conversão automatizada do Forms na Adobe Cloud](#connectafcadobeio).

### (Somente para desenvolvedores) Conecte sua instância local do AEM Forms ao serviço de conversão automatizada do Forms na Adobe Cloud {#connectafcadobeio}

Depois que um administrador fornecer acesso ao desenvolvedor, você poderá conectar sua instância local do AEM Forms ao serviço de conversão automatizado do Forms em execução na Adobe Cloud. Execute as seguintes etapas na sequência listada para conectar sua instância do AEM Forms ao serviço:
...
O serviço de conversão automatizado da Forms usa o serviço de correio Day CQ para enviar notificações por email. Essas notificações por email contêm informações sobre conversões bem-sucedidas ou com falha. Se você escolher não receber notificação, ignore essas etapas. Execute as seguintes etapas para configurar o Day CQ Mail Service:
...
Especifique um endereço de email no perfil do usuário AEM designado para executar o serviço. Verifique se o usuário é o membro do grupo de usuários [de](https://helpx.adobe.com/experience-manager/6-4/forms/using/forms-groups-privileges-tasks.html) formulários. Os emails são enviados para o endereço de email do usuário que está executando a conversão. Para especificar um endereço de email para o usuário e adicionar o usuário ao grupo de usuários`e formulários:

1. Faça logon na instância do autor do AEM Forms como um administrador AEM. Use suas credenciais AEM locais para fazer logon. Não use o Adobe ID para fazer login. Tap **[!UICONTROL Adobe Experience Manager]** > **[!UICONTROL Tools]** > **[!UICONTROL Security]** > **[!UICONTROL Users]**.
...
Um certificado público permite que você autentique seu perfil na E/S do Adobe.
...
1. Toque **[!UICONTROL Download Public Key]** e salve o arquivo de certificado *AEM-Adobe-IMS.crt* em seu computador. O arquivo de certificado é usado para [configurar as APIs de serviço no Console](#createintegration)do desenvolvedor do Adobe. Tocar **[!UICONTROL Next]**.
...
Para usar o serviço de Conversão automatizada do Forms, crie um projeto e adicione a API do serviço de configuração automatizada do Forms ao projeto no Adobe Developer Console. A integração gera a chave da API, o segredo do cliente, a carga (JWT).

1. Faça logon em [https://console.adobe.io/](https://console.adobe.io/). Use sua conta de desenvolvedor Adobe ID que o administrador provisionou para fazer login no console de E/S do Adobe para fazer login.
...
1. Toque em **[!UICONTROL Service Account (JWT)]** para visualização da chave da API, do segredo do cliente e outras informações necessárias para conectar sua instância AEM local ao serviço de conversão automatizada da Forms. As informações na página são usadas para criar a configuração IMS na máquina local.
...
1. Em sua instância do AEM Forms, toque em **[!UICONTROL Adobe Experience Manager]** > **[!UICONTROL Tools]**> **[!UICONTROL Cloud Services]** > **[!UICONTROL Automate Forms Conversion Configuration]**.
1. Toque na **[!UICONTROL Global]** pasta e toque **[!UICONTROL Create]**. A página para criar a configuração de conversão automatizada do Forms é exibida. A configuração é criada na pasta Global. Você também pode criar a configuração em uma pasta diferente que já existe ou criar uma nova pasta para suas configurações.
...
1. Na **[!UICONTROL Create Automated Forms Conversion Configuration]** página, especifique o valor para os seguintes campos e toque em **[!UICONTROL Next]**.
   ...
   | URL do serviço | URL do serviço de conversão automatizada da Forms na Adobe Cloud. Use o `https://aemformsconversion.adobe.io/` URL. |
