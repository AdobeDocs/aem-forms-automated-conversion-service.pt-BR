...
1. **O serviço pode ser instalado no local?**
   <p>A Adobe treina os algoritmos AI e ML do serviço de Conversão de formulários automatizada regularmente com novos conjuntos de dados para melhorar a precisão da conversão. Os algoritmos atualizados são implantados no serviço de conversão em execução na Adobe Cloud em intervalos periódicos. Todos os clientes do serviço são beneficiados pelos algoritmos atualizados. Portanto, a implantação central hospedada na nuvem é mais adequada para o serviço de Conversão de formulários automatizados para aprender e fornecer continuamente melhorias a todos os clientes.</p> 
    <p>O serviço converte formulários em branco em formulários adaptáveis. O serviço não oferece suporte para formulários preenchidos e extração de dados de formulários preenchidos. Remova dados de formulários preenchidos e remova ou remova informações proprietárias da lista de permissões dos formulários antes de enviar os formulários para o serviço de conversão</p> <br>

1. **O serviço oferece suporte a todos os formatos de formulários PDF? Quais são os idiomas suportados?**
...
1. **O serviço pode converter um formulário do Microsoft Word em formulários adaptáveis?**
   ...
   1. Use o Adobe Acrobat para [converter os formulários PDF produzidos em formulários](https://helpx.adobe.com/acrobat/how-to/convert-word-excel-paper-pdf-forms.html)PDF preenchíveis.
   1. Use o Adobe Acrobat para atualizar e corrigir campos de formulário manualmente.
   1. Salve o formulário PDF. Agora, você pode usar o formulário com o serviço de conversão para gerar um formulário adaptável. Também é possível usar o formulário como um modelo Documento de registro.
...
1. **O serviço suporta formulários XDP vinculados ao esquema? Se eu tiver um XDP vinculado a um esquema, será necessário incorporar o esquema ao XDP?**
   <p>Sim, o serviço oferece suporte a formulários XDP vinculados ao esquema e exige que o esquema seja incorporado ao formulário XDP de origem. Quando você converte um formulário XDP vinculado ao esquema, o serviço gera um esquema JSON. O esquema JSON é estruturalmente semelhante ao esquema XSD dos formulários XDP de origem.</p> <br>
...
Os motivos mais comuns para a conversão falhar são:</p>
   * Formulários PDF protegidos são fornecidos para a conversão. Não use formulários PDF protegidos por senha ou protegidos para conversão.
...
1. **Algumas correções são repetidas nos formulários. O serviço pode identificar e corrigir todas essas instâncias em conversões futuras?**
   ...
   O serviço está consistentemente treinando em seus formulários e padrões. Aprende novos padrões diariamente. Ainda é necessário iniciar a aplicação automática de correções repetidas nos formulários. Fique de olho nos formulários de pré-lançamento para a disponibilidade de tal recurso. <br/><br/>
